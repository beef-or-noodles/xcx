				...
				<block v-for="(day, index) in weeks" :key="index">
					<view
						:class="{
							...
							'uni-calender__date-current': (day.date == canlender.date || day.checked) && canlender.month == day.month && !day.disable,
...
.uni-calender__body-date-week {
	...
	.uni-calender__date {
		...
		&.uni-calender__date-current {
			...
			.uni-calender__circle-box {
				background: rgba(79, 117, 231, 1);
				box-shadow: 0px 6px 10px -5px rgba(0, 54, 213, 0.8);
			}
