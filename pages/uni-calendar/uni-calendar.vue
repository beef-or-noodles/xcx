			...
			<view class="uni-calendar__wrapper">
				<view class="uni-calenda__content">
					<view class="uni-calendar__panel">
						<!-- 		<view class="uni-calendar__date-befor" @tap="dataBefor('0', 'month')"><text class="iconfont icon-jiantou" /></view>-->
						...
						<!-- <view class="uni-calendar__date-after uni-calendar__rollback" @tap="dataBefor('1', 'month')"><text class="iconfont icon-jiantou " /></view>
						 -->
						<view class="uni-calendar__backtoday" @tap="switcherBtn">{{ switcher ? '收起' : '展开' }} 
						<span class="arrow" :class="{dowm:switcher}"><img src="./arrows.png" alt=""></span> 
						</view>
					</view>
					...
					<view class="uni-calendar__header">
						<view class="uni-calendar__week">周日</view>
						<view class="uni-calendar__week">周一</view>
						<view class="uni-calendar__week">周二</view>
						<view class="uni-calendar__week">周三</view>
						<view class="uni-calendar__week">周四</view>
						<view class="uni-calendar__week">周五</view>
						<view class="uni-calendar__week">周六</view>
					</view>
					<uni-calendar-item v-if="slide === 'none'" :canlender="canlender" :lunar="lunar" @selectDays="selectDays" />
					<swiper
						v-else
						:style="{ height: domHeihgt + 'px' ,overflow:'hiden'}"
					...
					>
						<swiper-item v-for="(item, itemindx) in swiperData" :key="itemindx">
							<view :class="elClass"><uni-calendar-item :canlender="item" :lunar="lunar" @selectDays="selectDays" /></view>
						</swiper-item>
					</swiper>
				</view>
			</view>
...
export default {
	...
	watch: {
		canlender(val){
			this.canlenderNew = JSON.parse(JSON.stringify(this.canlender));
			this.open(this.switcher);
		},
		selected(newVal) {
			let canlender = this.canlender;
			// 打卡之后更新信息
			this.isSilde = true;
			this.getMonthAll(this.currentIndex, canlender.year + '-' + canlender.month + '-' + canlender.date);
		},
		switcher(val) {
			this.open(val);
		}
	...
	methods: {
		...
		switcherBtn() {
			...
			let height = this.domHeihgt / 6; //每行高度
			if(!this.switcher){
				this.domHeihgt = height * 1;
			}else{
				this.domHeihgt = this.domHeihgt*6;
			}
		},
		
		open(val){
			let canlender = this.canlender;
			let weekData = this.canlender.weeks;

			if (!val) {
				weekData.forEach((weeks, week) => {
					weeks.forEach((day, index) => {
						if ((day.date == canlender.date || day.checked) && canlender.month == day.month && !day.disable) {
							this.canlender.weeks = [];
							this.canlender.weeks.push(weeks);
					...
					this.canlender.weeks = [];
					this.canlender.weeks.push(...this.canlenderNew.weeks);
		...
		animationfinish(e) {
			...
			let date = this.swiperData[index].year + '-' + this.swiperData[index].month + '-' + this.swiperData[index].date;
		...
		getMonthAll(index, date, first) {
			...
			let beforeyear = canlender.year + '-' + canlender.month + '-' + (canlender.date > tempbefore.lastDate ? tempbefore.lastDate : canlender.date);
			let afteryear = canlender.year + '-' + canlender.month + '-' + (canlender.date > tempafter.lastDate ? tempafter.lastDate : canlender.date);
		 ...
		 */
		selectDays(params) {
			let { week, index, ischeck, isDay } = params;
			let canlender = null;
			if (this.slide === 'none') {
		...
		getQueryDom(index) {
			...
			dom.boundingClientRect(rect => {}).exec(e => {
				...
				if (e[0][index]) {
					if(!this.switcher){
							this.domHeihgt = e[0][index].height / 6;
							console.log(this.domHeihgt);
					}else{
						this.domHeihgt = this.domHeihgt * 1
						console.log(this.domHeihgt,"++++++++");
					}
	...
	.uni-calenda__content {
		.uni-calendar__panel {
			...
			.uni-calendar__backtoday {
				...
				height: 50rpx;
				line-height: 50rpx;
				border: 1px rgba($color: #0036D5, $alpha: 0.5) solid;
				border-right: none;
				font-size: 24rpx;
